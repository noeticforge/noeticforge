...
3. **mock 对自动起标题请求恒定回复「默认会话」且不写 `last-request.json`**（见 `mock-openai.mjs` 路由注释）：v0.5.7 起
   默认会话也会被自动起标题——若无此规则，起标题请求会①把会话改成 mock 兜底回显的乱码名导致 J8 找不到「默认会话」，
   ②覆盖 J11 正在断言的请求捕获文件。改 mock 路由前先想清楚对这两处的影响。
