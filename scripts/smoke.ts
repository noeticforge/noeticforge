...
async function main(): Promise<void> {
  ...
  const BUILTIN_PLUGINS = ['read-file', 'write-file', 'shell-exec', 'web-fetch', 'kb', 'ask-user', 'system-master', 'm3e-canvas'];
  check(
    report.loaded.length === BUILTIN_PLUGINS.length && BUILTIN_PLUGINS.every((n) => report.loaded.includes(n)),
    `${BUILTIN_PLUGINS.length} 个内置插件全部加载成功（含 kb、ask-user、system-master 及 m3e-canvas 画布设计器）`,
  );
