...
- **会话沉淀**：模型可通过 `kb.archive` 工具将当前对话要点、排障结论或方案一键归档为 Markdown 卡片（存入 `知识库/会话归档/` 等分类），自动刷新索引，沉淀后立即可查
- **向量模型**：任意 OpenAI 兼容 `/v1/embeddings` 服务，默认预设 **VTXAI/vtx-embed-7M**（超轻量 4.7MB 代码 embedding，HF 自动拉取缓存）；运行 `npm run serve:vtx` 一键启动本地服务；服务不在线时**自动降级纯关键词**，检索永不断供
- **可配置**：知识库目录 / 切块策略 / 向量开关 / 服务地址 / 模型名全部在「设置 → 插件 → 知识库」
- 索引存 `知识库/.kb-index.json`（自动构建，文件变动自动失效重建；`kb.reindex` 强制重建）

```bash
# 启动本地向量服务（可选，不启动则自动降级为关键词检索）
